...
// Ahora lee y escribe anuncios en Firestore a través de FirebaseHelper.
public class ZonaActivity extends BaseDrawerActivity {

    private FirebaseHelper fb;
    private SessionManager session;

    private String miId;        // UID de Firebase Auth (antes era long de SQLite)
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // Cargamos la provincia del usuario desde Firestore antes de mostrar anuncios
        fb.obtenerUsuario(miId, yo -> {
    ...
    // Consulta Firestore y almacena todos los anuncios de la provincia del usuario
    private void cargarAnuncios() {
        if (miProvincia == null || miProvincia.isEmpty()) {
            mostrarMensaje("No tienes provincia asignada. Actualiza tu perfil.");
            return;
        }
        // Carga asíncrona desde Firestore; la UI se actualiza en el callback
        fb.obtenerAnunciosProvincia(miProvincia, anuncios -> {
    ...
    private void agregarTarjetaAnuncio(Map<String, Object> anuncio) {
        ...
        // Firestore devuelve números como Long; usamos Number para evitar ClassCastException si es null
        long timestamp = anuncio.get("timestamp") instanceof Number
                ? ((Number) anuncio.get("timestamp")).longValue() : 0L;
    ...
    private void mostrarFormularioPala() {
        ...
        btnPublicar.setOnClickListener(v -> {
            ...
            // Publicamos en Firestore de forma asíncrona; el callback actualiza la UI
            fb.publicarAnuncio(miId, "PALA", sb.toString(), miProvincia, ok -> {
                if (ok) {
    ...
    private void mostrarFormularioClase(boolean esSolicitar) {
        ...
        btnPublicar.setOnClickListener(v -> {
            ...
            fb.publicarAnuncio(miId, tipo, sb.toString(), miProvincia, ok -> {
                if (ok) {
                    Toast.makeText(this, getString(R.string.zona_publicado), Toast.LENGTH_SHORT).show();
                    if (dialogRef[0] != null) dialogRef[0].dismiss();
                    cargarAnuncios();
    ...
    private void mostrarFormularioPartida() {
        ...
        btnPublicar.setOnClickListener(v -> {
            ...
            fb.publicarAnuncio(miId, "PARTIDA", sb.toString(), miProvincia, ok -> {
                if (ok) {
                    Toast.makeText(this, getString(R.string.zona_publicado), Toast.LENGTH_SHORT).show();
                    if (dialogRef[0] != null) dialogRef[0].dismiss();
                    cargarAnuncios();
